...
func (c *client) Reload() error {
	...
	if version != c.load().ProjectVersion {
		...
		if err != nil {
			...
			_ = c.addAgentStatus(agent)
			return err
		}
		agent.Status = types.AgentStatusSuccess
	}
	return c.addAgentStatus(agent)
}
...
func (c *client) addAgentStatus(agent types.Agent) error {
	...
	if resp.StatusCode != http.StatusOK {
		return fmt.Errorf("unexpected status code for %s: %s (%d) %s", c.cfg.GetUrlApiAgents(), resp.Status, resp.StatusCode, resp.Body)
	}
	return nil
}
